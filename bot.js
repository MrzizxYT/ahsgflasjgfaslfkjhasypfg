...
.addField('     **--crcolors** ' , '**لـ عمل 50 لون** ')
...
guild.channels.find('id', '465682557187653644').setName(" Voice「"+rebel+"」");
  client.setInterval(() =>{
    let d = Date.now()
  }, 5000);
});
...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.get("463823539045400588");
...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.get("464177539791847453");
    if (!channel) {
